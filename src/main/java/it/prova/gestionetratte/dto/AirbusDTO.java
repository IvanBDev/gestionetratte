...
public class AirbusDTO {
	...
	public AirbusDTO(Long id, @NotBlank(message = "{codice.notblank}") String codice,
			...
			@NotNull(message = "{numeroPasseggeri.notnull}") @Min(1) Integer numeroPasseggeri) {
		super();
		this.id = id;
		this.codice = codice;
	...
	public static List<AirbusDTO> createAirbusDTOListFromModelList(List<Airbus> modelListInput, boolean includeTratta) {
		return modelListInput.stream().map(airbusEntity -> {
			AirbusDTO result = AirbusDTO.buildAirbusDTOFromModel(airbusEntity, includeTratta);
			if(includeTratta)
